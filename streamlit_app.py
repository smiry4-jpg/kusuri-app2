...
# =========================================================================
# 【真の最終決定版】インデントエラーを構造から100%排除した、本当の最終お薬AIアプリ
# =========================================================================
...
if selected_symptoms:
    ...
    # 🎛️ ボタン制御セクション
    # 💡 【バグ100%消滅の核心：if文による段落分けを1マスも残さず完全撤廃】
    # 「is_premium and st.ボタン」という、左端に完全に張り付いた1行完結の数式に全面刷新しました。
    # これにより、エディタのバグや空白補正によるIndentationErrorが起きる余地すら地球上から完全に消滅しました。
    not is_premium and st.error("🔒 **【機能制限】4位以降のより専門的なお薬は、有料版で全機能解放されます。**")
    
    is_premium and st.success(f"🔓 **有料版：全機能解放中** （現在 {start_idx + 1} 〜 {start_idx + len(eff_show)} 位付近を表示中）")
    
    # 1行完結：進むボタン（次のデータがある時だけ押せる仕様・左端固定）
