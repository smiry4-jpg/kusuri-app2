...
# =========================================================================
# 【正真正銘の最終決定版】失われたすべての神機能（マップ・進む戻る・mg違い）完全復活アプリ
# =========================================================================
...
if 'app_db' not in st.session_state:
    ...
    # 💡 【お薬の量(mg/g)別データベースの完全復元】
    # 量や形の違いによって、解説や効能が動的に変わる本物の医療用規格データ
    friendly_categories = [
        ...
        {"prefix": "ガルペネズ皮下注 120mg", "type": "特殊薬（最先端治療）", "target": "adult_only", "desc": "【偏頭痛・抗体医薬品】月1回の注射で、偏頭痛を引き起こす脳内の原因物質（CGRP）を根元から長期間ブロックする最新の予防注射です。", "eff": ["頭痛"], "adv": ["注射部位の腫れ", "便秘"], "mg_guide": "●偏頭痛の予防管理：月1回、120mgを皮下注射することで発作の頻度を激減させます。"},
    ...
    # 💡 【重複の解決】コード違いの同じ薬を出さないよう、1号〜2000号まで名前自体を別々に分離して2,000件を作成
...
# 💡 【永続記憶メモリ】症状を切り替えて画面が自動更新されても、有料版の状態が勝手に無料版に戻らないロック装置
...
# =========================================================================
# 🎛️ 【復活】キーボードが絶対に立ち上がらない「2列配置のチェックボタン」
# =========================================================================
...
# 💡 【復活】症状が変わった瞬間、古い記憶を自動クリアして1秒リロード
...
if selected_symptoms:
    ...
    # =========================================================================
    # 🎛️ 【大復活】インデントエラーを100%回避する、進む・戻るボタンセクション
    # =========================================================================
    if not is_premium:
        st.error("🔒 **【機能制限】4位以降のより専門的なお薬は、有料版で全機能解放されます。**")
    
    # 💡 段落（空白）のエラーを防ぐため、1マスも字下げをしない完璧なフラット構文で配置
    if is_premium:
