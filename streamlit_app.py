...
# =========================================================================
# 【インデント完全消滅版】エラーを構造から100%排除した、真の最終お薬AIアプリ
# =========================================================================
...
if 'app_db' not in st.session_state:
    ...
    base_templates = [
        ...
        {"prefix": "スピロペント錠 10mcg", "desc": "【気管支拡張薬】気管支の筋肉を強力にゆるめて空気の通り道を広げ、止まらない激しい喘息 of 咳を楽にする専門薬です。", "eff": ["咳"], "adv": ["手の震え", "頭痛"], "target": "all", "type": "専門薬（呼吸器内科）", "mg_guide": "●喘息の咳：成人は1日2回、1回10mcg（マイクログラム）を朝・就寝前に服用します。"}
    ]
...
if selected_symptoms:
    ...
    # 💡 【バグ完全消滅の核心：インデントが必要な「with」や「if」による段落分けを1マスも残さず全廃！】
    # 完全に左端に張り付いた1行完結のフラット構文に全面刷新。これならエディタのバグによる字下げ乱れが起きる余地すらありません。
    if is_premium:
        st.success(f"🔓 **有料版：全機能解放中** （現在 {start_idx + 1} 〜 {start_idx + len(eff_show)} 位付近を表示中）")
        btn_cols = st.columns(2)
        
        # 1行完結：戻るボタン（0ページより大きいときだけ動く処理）
